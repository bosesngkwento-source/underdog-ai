...
for (const key of ["DISCORD_TOKEN", "DISCORD_CLIENT_ID", "DISCORD_GUILD_ID", "OPENAI_API_KEY"]) {
...
const getCsvSetting = (key) =>
  (getSetting(key) || "").split(",").map(s => s.trim()).filter(Boolean);
...
function isStaff(interaction) {
  return Boolean(
    ...
    interaction.member?.roles?.cache?.some(role => staffRoles().includes(role.id))
  );
}

// High-rank task authority. Put Senior Mod/Admin/Owner role IDs in HIGH_RANK_ROLE_IDS.
...
function isHighRank(interaction) {
  return Boolean(
    interaction.memberPermissions?.has(PermissionFlagsBits.Administrator) ||
    interaction.member?.roles?.cache?.some(role => highRankRoles().includes(role.id))
  );
}
...
function searchMemories(query, limit = 8) {
  ...
  const where = words
    .map(() => "(lower(information) LIKE ? OR lower(category) LIKE ?)")
    .join(" OR ");
...
- You can sometimes be a little mean in a playful, obviously joking way, especially during boxing/game banter.
- You may use mild dark humor when appropriate, but never target protected traits, encourage real-world harm, make threats, or become seriously abusive.
- Balance the attitude: sometimes sarcastic/roasting, sometimes kind and supportive.
- Never bully a user relentlessly; if someone seems genuinely upset, switch to respectful/helpful behavior.
...
${memoryContext}`;
  ...
  return clip(response.choices?.[0]?.message?.content || "I don't know that yet.");
}

async function classifyForMemory(message) {
  const configuredChannels = getCsvSetting("memoryChannels");
  if (!configuredChannels.includes(message.channelId)) return null;
...
const commands = [
  new SlashCommandBuilder()
    ...
    .addStringOption(o =>
      o.setName("information").setDescription("Information to remember").setRequired(true))
    .addStringOption(o =>
      o.setName("category").setDescription("Memory category").setRequired(false)),

  new SlashCommandBuilder()
    .setName("forget")
    .setDescription("Remove a stored memory")
    .addIntegerOption(o =>
      o.setName("memory").setDescription("Memory ID").setRequired(true)),
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(o =>
      o.setName("instruction").setDescription("Task for the bot").setRequired(true)),

  new SlashCommandBuilder()
    .setName("announce")
    .setDescription("Post an announcement (high-rank staff only)")
    .addChannelOption(o =>
      o.setName("channel").setDescription("Announcement channel")
...
client.on("interactionCreate", async interaction => {
  ...
  try {
    const staffOnly = ["remember", "forget", "memories", "setchannel"];
    const highRankOnly = ["task", "announce"];
    if (highRankOnly.includes(interaction.commandName) && !isHighRank(interaction)) {
      return interaction.reply({ content: "Denied. Only authorized high-rank staff can give me server-management tasks.", ephemeral: true });
    }

    if (staffOnly.includes(interaction.commandName) && !isStaff(interaction)) {
      return interaction.reply({
        content: "Only authorized staff can manage server memory/settings.",
        ephemeral: true,
      });
    }
    ...
    if (interaction.commandName === "remember") {
      ...
      const category = interaction.options.getString("category") || "community";
